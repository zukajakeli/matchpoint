// Supabase Edge Function: flitt-event-callback
// Receives server-to-server POST from Flitt for event registration payments.
// Same signature verification as flitt-callback, but updates event_registrations.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error("flitt-event-callback: Failed to parse body");
    return new Response("OK", { status: 200 });
  }
  ...
  if (!receivedSig) {
    console.error("flitt-event-callback: No signature");
    return new Response("OK", { status: 200 });
  }

  const valid = await verifyFlittSignature(secretKey, body, receivedSig);
  if (!valid) {
    console.error("flitt-event-callback: Signature mismatch");
    return new Response("OK", { status: 200 });
  }
  ...
  if (!orderId) {
    console.error("flitt-event-callback: Missing order_id");
    return new Response("OK", { status: 200 });
  }
  ...
  if (!existing) {
    console.error(`flitt-event-callback: No registration for order_id ${orderId}`);
    return new Response("OK", { status: 200 });
  }
  ...
  if (error) {
    console.error("flitt-event-callback: Update failed:", error);
  }
